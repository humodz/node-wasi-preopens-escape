...
The `WASI` class has a `preopens` parameter that takes a directory. Usually, the WASM program is not be able to access files outside this directory, but it's possible to circumvent that if a symlink replaces the file at a very precise moment.
